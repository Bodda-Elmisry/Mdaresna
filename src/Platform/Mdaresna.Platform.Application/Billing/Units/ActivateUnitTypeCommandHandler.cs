...
public sealed class ActivateUnitTypeCommandHandler(
    ...
    IClock clock)
{
    public async Task<UnitTypeMutationResult> HandleAsync(
        ActivateUnitTypeCommand command,
        CancellationToken cancellationToken = default)
    {
        ...
        if (!unitType.Activate(now))
        {
            return new UnitTypeMutationResult(unitType.Id, unitType.Version, true, false);
        }

        audit.Stage(new UnitCommerceAuditRecord(
            Guid.NewGuid(), command.RequestedByAccountId, null,
            "platform.unit_type.activated", "unit-type", unitType.Id.ToString("D"),
            now, command.CorrelationId, null));
        await unitOfWork.SaveChangesAsync(cancellationToken);
        return new UnitTypeMutationResult(unitType.Id, unitType.Version, true, true);
    }
}
