...
public sealed record AcademicSettingItemResponse(Guid Id,Guid? ParentId,Guid? SecondaryParentId,Guid? BranchId,string Code,string NameAr,string NameEn,bool IsActive,bool IsDeleted,DateTimeOffset? DeletedAtUtc,DateOnly? StartDate,DateOnly? EndDate,int? SortOrder,string? ProgramType,string? EventType,bool? IsSchoolClosed,TimeOnly? StartsAt,TimeOnly? EndsAt,string? DayOfWeek);
