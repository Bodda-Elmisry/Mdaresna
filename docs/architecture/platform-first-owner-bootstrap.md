...
The tool creates one central Identity account with an **unverified** phone identifier and no password credential. It then creates a pending Platform-local user without a credential, the `app-manager` system role and assignment, and a Platform audit entry. The pending person cannot sign in or use Platform APIs.
...
The Flutter setup screen calls `POST /api/platform/v1/auth/first-owner/activation/start` with `{ "phone": "00967777661929" }`. The response is always generic so it does not reveal whether an account exists. The owner receives an eight-digit SMS code, enters it, then chooses a password of at least 12 characters. The client submits both in `POST /api/platform/v1/auth/first-owner/activation/complete` as `{ "phone": "...", "code": "...", "password": "..." }`. The API validates the Identity-owned phone challenge, establishes the hash **only in Platform**, then verifies the central phone and activates the person. The local write precedes the central commit: a retry can finish a partial central commit, while login remains closed until central verification succeeds. It does not issue a token; the owner then uses the normal `login` endpoint. Codes expire in ten minutes, permit five failed attempts, and are limited to one send per minute and ten sends per rolling day per account. IP-level rate limiting also applies.
